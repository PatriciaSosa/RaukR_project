...
ui <- page_navbar(
  ...
  nav_panel(
    ...
    card(
      ...
      girafeOutput("plate_plot", height = "600px")
    )
  ),
  nav_panel(
    "MFI QC",
    layout_column_wrap(
      width = 1,
      card(card_header("Raw MFI value vs control beads"), girafeOutput("mfi_controls")),
      card(card_header("Raw MFI value vs antigens"), girafeOutput("mfi_antigen")),
      card(card_header("Raw MFI value vs all samples"), girafeOutput("mfi_sample")),
      card(card_header("Pool reproducibility (CV%)"), girafeOutput("mfi_cv")),
      card(
        card_header("By antigen, faceted by sample type"),
        selectizeInput("facet_antigens", "Antigens", choices = NULL, multiple = TRUE,
                       options = list(placeholder = "All antigens if empty")),
        girafeOutput("mfi_facet")
      )
    )
  ),
  nav_panel(
    ...
    layout_column_wrap(
      ...
      card(
        ...
        checkboxInput("remove_low_mfi", "Remove wells flagged low MFI signal", value = FALSE),
  ...
  nav_panel(
    ...
    card(
      ...
      downloadButton("dl_report", "Download QC report (Markdown)", class = "w-100")
    )
  )
)
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    updateSelectizeInput(session, "facet_antigens", choices = ants, server = TRUE)
  })
  ...
  output$mfi_facet <- renderGirafe({
    ants <- if (length(input$facet_antigens)) input$facet_antigens else NULL
    plot_mfi_by_antigen_facet(loaded()$merged, antigens = ants)
  })
  ...
  qc_result <- eventReactive(input$apply_removal, {
    ...
    apply_qc_removals(
      ...
      remove_low_mfi = isTRUE(input$remove_low_mfi),
  ...
  output$dl_report <- downloadHandler(
    filename = "qc_report.md",
    content = function(file) {
      res <- tryCatch(qc_result(), error = function(e) NULL)
      writeLines(
        build_qc_report(loaded()$parsed, thresholds_r(), flags_r(),
